...
class ModelSampler:
    ...
    def predict_labels(self, dataset):
        ...
        for i,row in enumerate(dataset):
            ...
            ################################################## TODO: ##################################################
            truncated_response = response[len(prompt):].strip()
            # #TODO: Truncate Reponse to get predicted label (hint: the response is a string which includes input prompt plus the generated label)
            # first_20_chars = truncated_response[:20]
            #TODO: Get first 20 characters of truncated response
            if "positive" in truncated_response:
            # TODO: Check if "positive" is in first 20 characters of truncated response
                predicted_label = 1
            elif "negative" in truncated_response:
            #TODO: Check if "negative" is in first 20 characters of truncated response
                predicted_label = 0
            else:
                predicted_label = -1
